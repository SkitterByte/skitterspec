...
**Highlights:** A spec's Linear issue is now assigned to whoever is building it without anyone turning it on first, and released again when the spec completes. Teams whose PM owns assignment can decline it with one line.
...
### Sync
- **Action required** — A spec's Linear issue is now assigned to whoever is building it without anyone turning it on first, and released again when the spec completes. Teams whose PM owns assignment can decline it with one line.
...
- **New** — When /spec, /spec-bug or /spec-hotfix adopt an existing Linear issue, they now post the reporter's description back onto that issue as a comment before replacing it with the spec — and say so, so you know where the original went instead of being pointed at the issue's edit history.
- **New** — Adopting an existing Linear issue with /spec no longer loses what the reporter wrote. Their description is posted back onto the issue as a comment before the spec replaces it, so the original report stays readable next to the spec it became.
- **New** — A spec whose Linear link is lost — a bad merge, a hand edit, a stray script over its frontmatter — can now be pointed back at its issue with "spec-sync reattach", phases included. Pushing such a spec also stops silently creating a duplicate: it names the issue that already exists and tells you to adopt it, with --force-new when the repeated title is genuine.
- **New** — If a push creates the Linear issue but cannot record it — a read-only tree, a permission error — skitterspec now names the issue that exists and tells you to reattach it. It used to say "re-run to resume", which in that one case mints a second issue beside a perfectly good one.
- **New** — When Linear refuses a push, skitterspec now tells you what it actually refused and whether waiting could help. A free-plan issue cap used to arrive as "Linear API error: usage limit exceeded", which reads like a rate limit and sent an hour into checking one that was untouched; you now get Linear's own explanation, naming the cause and how to clear it. A refusal that wrote nothing also stops claiming it saved ids, so you can tell a fresh start from a resume.
- **New** — A project can now tell skitterspec it does not own a synced field, with "none" in sync.fieldOwnership — the field is left alone in the tracker rather than written or cleared.
- **Fixed** — Recording a spec's assignee now fails with a clear reason when the project does not sync that field, instead of writing a stamp and telling you to push it — the push would have silently dropped it.
...
- **New** — A new "/spec-list" command lists every spec Linear is tracking with its state, who holds it and the name to start it by, and still answers from the repo when Linear cannot be reached.
- **New** — You can now take a spec over, hand it back, or pass it to a teammate with /spec-claim, and its Linear issue follows. Handing over looks the person up by name or email rather than trusting a pasted id.
...
- **Fixed** — The Developer line on a spec now stays as your git name, matching the author line, the state log and your commits. Handing a spec to a teammate still names them, since there is no git name for them.
...
### Sync
- **Action required** — The CLI verb that works out what would be pushed to Linear is now called spec-sync plan, because it plans and writes nothing while spec-sync apply does the writing. Scripts calling the old name get an error telling them what to use instead.
...
- **New** — A spec that is in progress now shows which phase is live and who holds it, so "what is Jane on" is answered by the listing itself rather than by opening the issue.
- **New** — "spec-sync list --next 5" now shows the top of your Linear backlog in the same order the Backlog view does, and tells you when nothing is prioritised so you can read it as someone's manual ordering rather than a ranking.
- **New** — A new "spec-sync list" command shows every spec Linear is tracking with its id, title, state and the local spec name to start it by, so you can see specs that were authored on branches you do not have checked out.
- **New** — A spec's Linear issue can now be assigned to whoever is building it, and hands itself back when the spec completes. Turn it on with "assignee": "push" in linear.config.json; without it nothing changes. An issue somebody assigned in Linear is never overwritten.
- **New** — Skitterspec now works out who you are in Linear from your own API key, so nothing needs configuring. Run spec-sync whoami to see it, or spec-sync users to look someone up by name or email.

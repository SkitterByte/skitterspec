...
- [ ] Add `packages/common/assets/rules/negative-checks.md`. Keep it short — four
      points, each with the incident that earned it, in the register of
      `spec-planning.md`.
- [ ] State the four: prefer a positive signal to an absence; name what would
      blind the lookup, beside the check; pair every accusation with a
      stays-silent test; bias the unknown case toward inaction.
- [ ] Cite the in-repo example of the correct shape — `managedState`
...
- [ ] Cite the three real incidents by name so the rule reads as earned rather
      than invented: the archived-issue trap, the npm registry lag, and
      `bug-scaffold-empty-buckets`.
- [ ] Confirm it installs with no manifest edit — `listRules()`
      (`packages/common/src/init.js:26`) discovers the directory.
- [ ] Symlink it into this repo's `.claude/rules/` beside the other two, so the
      repo governs itself by the rule it ships.
- [ ] Verify the existing asset suite covers it (`packages/common/test/assets.test.js`

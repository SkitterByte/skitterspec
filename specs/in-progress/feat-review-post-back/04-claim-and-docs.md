...
- [ ] Add the claim path to `/spec-diff` §2, **ahead of** the paste path: a bare
      six-digit number in the message is a claim, and the skill runs
      `--claim <code>` rather than asking for the blob.
- [ ] Route the claimed pass into the existing §2 steps unchanged — the verdict
...
- [ ] Say what it cost: a claimed pass never entered the context, so the intake
...
- [ ] Keep the paste path documented as equal, not legacy — it is the whole story
      on `file://` (Decision 7).
- [ ] Report pending passes on render: `2 passes waiting — claim one with its
...
- [ ] Keep the `/spec-diff` description inside the 500-char budget; add a claim
      trigger only if a bare code genuinely needs to route.
- [ ] Tests: the skill prose pins the claim path and that it does not displace the

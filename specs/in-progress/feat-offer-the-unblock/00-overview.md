...
- 2026-09-18 — Spec created. Grew out of `bug-live-press-does-nothing`
  ...
  offered one.
- 2026-09-18 — Phase 3: the rule cites `/allow-main` alone, not `/spec-skip`.
  `/spec-skip` ships on `bug/live-press-does-nothing`, which is committed and
  unlanded, and `assets-prose.test.js` refuses an asset naming a `/spec-…` that
  does not ship. Worth adding once that branch lands.

...
- [x] Rewrite `packages/common/assets/skills/spec-init/SKILL.md:25` — drop
      ...
      skills".
- [x] Remove the `/spec-ready` row from `packages/common/README.md` (line ~20)
      and fix `packages/skitterspec/README.md:81` if its mention is not
      historical context.
- [x] Add `spec-ready` (and `spec-env`/`spec-env-down`, retired at the same
...
- [x] Rebuild dists (`pnpm build`) so `packages/skitterspec*/assets` pick up the
      corrected skill.
- [x] Add/extend tests covering this phase (the docs-claims guard above, plus a

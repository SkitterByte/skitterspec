...
- [x] Add `release: { stages: [] }` to `DEFAULT_CONFIG` and `defaults()` in
      `packages/linear/src/config.js`, with the comment naming why the ladder is
      project-owned rather than shipped.
- [x] Validate the block on load: `stages` is an array of `{key, state}`, both
...
- [x] Extend `validateStates` so `release.stages[].state` names are checked
      against the workspace alongside `config.states`, and reach all three call
      sites (`cli-sync.js:324`, `:450`, `:1768`).
- [x] Extend `spec-sync states` output to list the declared ladder next to the
      bucket map, so one command shows the whole state vocabulary.
- [x] Add the `release` block to `linear.config.json.example` and document it in
      `linear.config.md` (fields, ordering semantics, and that absence = off).
- [x] Tests: valid ladder loads; malformed entries rejected with a useful
...
Done 2026-09-04. Full suite green: **1118/1118** (1101 before, +17 new).

`stateSuggestions` gained a `label` field (`states.complete` /
`release.stages[test]`) so one printer serves both vocabularies. That change
broke `init-config`'s refusal branch, which still referenced `bucket` for its
`--state <bucket>="…"` remediation line — caught by the existing suite, fixed by
keeping `bucket` in the destructure and gating the remediation on it. A ladder
rung has no such flag and never carries a suggestion, so the line correctly
never fires for one.

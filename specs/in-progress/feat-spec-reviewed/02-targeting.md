...
- [x] Accept a **spec name** and act on that spec rather than the resolved one.
- [x] Accept a **tracker id** as a provider seam: resolve it through the
...
- [x] Compare the target's worktree against where the session stands, resolving
...
- [x] **Same tree — carry on**, with nothing said. This is the ordinary case and a
      line about it would be narration.
- [x] **Different tree — ask, then relocate.** Name both, say why it matters (an
...
- [x] **On a no, stop without claiming.** A pass claimed here and acted on there
      is the split this guard exists to prevent; claiming first and then refusing
      to move would spend the code for nothing.
- [x] **A target with no worktree is a refusal**, naming it: there is nowhere for
      an approve to commit. Suggest `/spec-start <name>` and stop.
- [x] Confirm the move landed before acting — `skitterspec spec-env resolve` with
...
- [x] Tests: the prose pins the ask-then-relocate rule, the no-worktree refusal,
      and that a "no" claims nothing; a guard that it does not simply act from the
      wrong tree.
- [x] **Stays silent:** a bare invocation, and a named one that resolves to the

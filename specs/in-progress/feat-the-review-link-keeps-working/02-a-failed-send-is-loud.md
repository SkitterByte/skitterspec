...
- [x] Move the send's failure paths out of `copy-hint` — a small grey line in the
...
- [x] Style the failure variant from `--del-bg` / `--del-fg` / `--del-mark`, the
...
- [x] **Name the cases separately**, because they send the reader to different
      places:
      ...
      - a rejected request — the network, or the server is down. Re-open, or use
        the command.
- [x] Offer the `/spec-reviewed <verdict>` command in the box, with its Copy
...
- [x] **Do not mark the page decided on a failure.** Nothing was delivered, so
...
- [x] Keep the clipboard blob path for a pass carrying marks: a command cannot
      carry notes, and a failed send must not become the moment they are dropped.
- [x] Tests: a `404` says the server is gone and offers the command; a `422`
...
- [x] Stays-silent test: a **successful** send is untouched — same callout, same
...
- `#send-failed` is a sibling of the decided panel, not the panel itself —
  `.decided.failed` for the shape, the delete palette for the colour, and
  deliberately no `is-decided`, so the diff stays and every verdict button stays
  pressable for a second try.
- `sendFailed(json, verdict, what, note)` is the one entry point, and the three
  callers name the cases: `404` says the server is gone and to re-open, `422`
  relays the engine's text as the whole note with no re-open advice, and a
  rejected `fetch` gets a third sentence naming neither.
- The command row reuses the `.sent-cmd.act` shape with `canCopy()` deciding
  whether there is a button, so there is one command control on the page rather
  than two kinds.
- `clearSendFailure()` runs at the top of `post` and again on a successful
  response, so a retry replaces the account rather than stacking on it.
- `failBox.scrollIntoView()` on failure: the verdict bar is at the foot of the
  page and this panel is at the head of it, so announcing without it would be
  the same bug one scroll position removed.

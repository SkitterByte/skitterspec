...
- [x] Update `packages/common/assets/rules/spec-planning.md:48` — it currently
...
- [x] Update `packages/common/README.md:139` — "**It builds the branch there and
      prints the path** — your session never moves" — and the hand-off sentence
      that follows it at line 143.
- [x] Check `packages/common/assets/core/env.config.md` for the same claim and
      update it if present; it is in the one-path test's `SURFACES` set, so a
      miss there fails that test rather than shipping.
- [x] Add the behaviour change to the **pending** `MIGRATION.md` entry for the
...
- [x] Run `scripts/docs-claims.test.js` and `scripts/migration-guide.test.js`
      early rather than at the end — both assert on the docs this phase edits.
- [x] **Discovered:** `MIGRATION.md` could not join the one-path test's
      `SURFACES` set — that set also guards `open.command`, and the entry has to
      name that key to tell upgraders to delete it. Guarded in
      `assets-kickoff-surfaces.test.js` instead, narrowed to `EnterWorktree`.
- [x] **Discovered:** `docs/index.html` needed no edit — its `/spec-start` row
      already says "offer to build phase 1 there", which is still true.

...
- [ ] In `packages/common/assets/skills/spec/SKILL.md`, add the `## Impact`
...
- [ ] Add the "required section, rows only for real changes; one-liner when no
      external surface" rule to the template comment.
- [ ] In Phase A/B prose, note the Impact table is derived from Phase A items 3
...
- [ ] In `packages/common/test/assets.test.js`, add a test asserting the `spec`

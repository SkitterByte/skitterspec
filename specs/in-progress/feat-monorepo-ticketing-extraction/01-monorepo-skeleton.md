...
- [x] Convert the repo to **npm workspaces**: private root `package.json`
...
- [x] Create `packages/common` from `main`'s base: move `bin/`, `src/` (incl.
...
- [x] Create `packages/sync-core` (`@skitterbyte/skitterspec-sync-core`, private):
...
- [x] Fix up cross-package `require`s and workspace symlinks (`npm install`); make
      sure `common`'s Linear code (still present this phase) resolves `sync-core`.
- [x] Run `node --test` across the workspace — green before the phase is done.

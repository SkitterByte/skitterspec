...
- [ ] Write the banner once, as a blockquote placed directly under each skill's
...
- [ ] Apply it to all 17 — the 11 in `packages/common/assets/skills` and the 6
      in `packages/linear/assets/skills`. Identical text everywhere; a per-skill
      variation is a thing to keep in step for no gain.
- [ ] Extend `assets-report-contract.test.js`: every skill in its list carries
      ...
      presence.
- [ ] Assert the banner text is **identical** across all 17, comparing them to

...
- [x] `config.js`: allow `assignee` in `sync.fieldOwnership` (the loader already
...
- [x] `normalize.js`: read `linear_assignee_id` and `linear_assignee_name` from
      the overview frontmatter into the local snapshot, beside `linear_identifier`.
- [x] `push.js` `projectionOf`: add `assignee`, derived from the bucket — the
...
- [x] `compare.js`: add `assignee` to `specIssueFieldHashes` and emit an assignee
...
- [x] Write the blind-spot comment beside that check, naming what would fool it:
...
- [x] `api.js`: send `assigneeId` on `issueUpdate` (and `issueCreate` where the
      plan carries one); `null` clears. `mcp.js` / the apply path: same field on
      `save_issue`.
- [x] `cli-sync.js` `apply`: carry the assignee op through to whichever adapter
      is in play, and include it in the read-back verify.
- [x] `spec-sync status`: add one drift line when Linear's assignee differs from
...
- [x] Add tests: assign on an in-progress spec; clear on `complete` and
      ...
      present and absent.
- [x] **Stays-silent tests**, one per way this could accuse the innocent: (a) an
...
- [x] Run `npm test` — green before the phase is done.

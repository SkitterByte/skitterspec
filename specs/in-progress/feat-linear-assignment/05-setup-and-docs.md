...
- [ ] `/spec-linear-setup`: add an interview step offering assignment ("assign the
...
- [ ] `doctor.js`: add an identity check following the existing check shape — the
...
- [ ] Document `sync.fieldOwnership.assignee` in
...
- [ ] Update `packages/linear/assets/core/SETUP.md` with the identity step and
      `/spec-claim`.
- [ ] Update the ticketing-provider paragraph in
...
- [ ] Add tests: doctor reports identity and stays silent when the field is not

...
- [x] Disable `✓ Commit & Continue` when `data.phases` says `hasNextPhase` is
      ...
      phone.
- [x] **Leave it enabled when the engine could not tell** — no `phases` key at
...
- [x] **The open-note block still wins.** A note open disables both committing
...
- [x] Do **not** touch `✓ Commit`: a clean read still commits (Decision 4), and a
      completed spec is still a legitimate thing to read.
- [x] Keep the page a **static artefact**: it reasons from the data it was
...
- [x] **Stays silent:** a spec mid-flight with phases left renders exactly as it

...
- [ ] Add `--write` to `doctor`. Refuse on a dirty git tree, naming the offending
...
- [ ] Repair together, never partially: `config.linear.teamKey`; the
      `linear_identifier` / `linear_issue_id` / `linear_url` frontmatter stamps;
      and the snapshot filenames under `sync.baseDir`.
- [ ] `git mv` the snapshot files so history survives — never write-and-delete.
- [ ] Rename the identifier-keyed entries **inside** each snapshot too. The
      hashes are content-derived and stay valid; only their keys move.
- [ ] Refuse to repair any ref that phase 2 classified as **missing** — repair
      what resolves, report the rest, exit non-zero so a caller notices.
- [ ] Extend `packages/linear/test/cli-doctor.test.js`: a dirty tree is refused
      ...
      and reported.
- [ ] Surface `doctor` in the `/spec-sync` skill from phase 1 — read by default,

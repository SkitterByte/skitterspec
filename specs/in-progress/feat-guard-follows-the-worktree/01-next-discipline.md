...
- [x] Rewrite §3's remote-build paragraph so its condition is **the resolved
...
- [x] Ungate `--record-primary` with it. The baseline is what §4b reads, so a
      rung-4 build that skipped it gets `cannot tell` instead of an answer —
      which is safe, and useless.
- [x] Say **how to compare**: take the `worktree:` line from
...
- [x] Rewrite §4b's heading and its `Only when…` line off the flag. The existing
...
- [x] Leave the rest of §4b **exactly as it is**: the three verdicts, the ban on
...
- [x] Fix the frontmatter description. *"builds one elsewhere only when handed
...
- [x] Leave §1 untouched. All four rungs, the refusal, the several-worktrees
...
- [x] Add the blind-spot comment beside the check (`negative-checks.md` rule 2):
      ...
      worktree.
- [x] Update the three assertions in
      ...
      make pass.
- [x] Keep the `--worktree` path's own assertions green untouched — it is still
      ...
      with.
- [x] Add the **stays-silent** cases (`negative-checks.md` rule 3): standing in
...
- [x] Run `pnpm test` at the repo root — green before the phase is done. Then
      ...
      **2121 passed, 0 failed**; composed output is gitignored, so the build

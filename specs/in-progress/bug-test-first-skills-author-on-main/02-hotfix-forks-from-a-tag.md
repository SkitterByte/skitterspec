...
- [ ] `packages/common/src/cli.js`: accept `--from <ref>` in the flag parser,
...
- [ ] Refuse `--from` where it cannot be honoured rather than ignoring it: a ref
      git does not know, or a spec that already resolves.
- [ ] Rewrite `packages/common/assets/skills/spec-hotfix/SKILL.md` §3 on phase
      1's shape, with the tag on the `up` line and the hand-move deleted.
- [ ] Tests: extend the phase-1 asset test to `/spec-hotfix`, and add engine
      ...
      changes.
- [ ] Green: `node --test packages/common/test/assets-test-first-authors-in-worktree.test.js`
      plus the new CLI test
- [ ] `node scripts/build-dist.js all`, then the full suite: `pnpm test`

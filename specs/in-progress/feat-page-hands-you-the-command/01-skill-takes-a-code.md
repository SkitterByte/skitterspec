...
- [ ] Accept a **six-digit code** as a third argument shape beside a spec name
      and a tracker id. Nothing else matches `^\d{6}$`, so the parse needs no
      flag and no ambiguity survives.
- [ ] On a code, resolve the spec as a **bare** invocation does — the worktree
...
- [ ] **Skip the offer-and-confirm step.** A code pasted from the operator's own
...
- [ ] A code that matches nothing **refuses and names nothing**, exactly as
      `--claim` does — listing the waiting codes would hand a guesser the answer
      whichever door they came through.
- [ ] **Keep the verify-by-echo path intact** for a bare invocation: the operator
      says a pass is waiting, the agent names a code back. Pasting is the faster
      path, not the only one.
- [ ] Say in the skill body **why a pasted code needs no verification**, so a
      later edit does not restore the round-trip as a safety measure it is not.
- [ ] Tests: the prose pins the code shape, the bare-resolution rule, the skipped
...
- [ ] **Stays silent:** a bare invocation behaves exactly as it does today

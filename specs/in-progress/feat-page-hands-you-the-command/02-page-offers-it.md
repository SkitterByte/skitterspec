...
- [x] Show the whole command where the "Sent" hint already is, and make it the
      thing the reader acts on. Not a second bar: that is where their eye is at
      the moment they need it.
- [x] Offer **Copy** when `navigator.clipboard.writeText` exists, reusing the
      page's existing clipboard path rather than a second implementation.
- [x] **Fall back to pre-selected text when it does not** — the usual case on a
      ...
      happened.
- [x] **Never show a Copy button that cannot copy.** Decide from
...
- [x] Keep the `file://` path exactly as it is: no server, no code, no command —
      it copies the blob and the operator pastes it, which is the whole story for
      a local reader.
- [x] Tests, driving the real page as `assets-review.test.js` establishes: with a
...
- [x] **Stays silent:** a page that sent nothing shows no command, and the

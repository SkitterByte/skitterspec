...
- [x] Create `packages/common/assets/skills/spec-next/SKILL.md` from `spec-go`'s
...
- [x] In-flight resolution, in order: the live spec of this checkout (receipt);
      ...
      real code.
- [x] Description within the 500-char budget with `Use when` triggers ("build
      the next phase", "continue the spec").
- [x] Leave `spec-go` untouched this phase — both exist on the branch until
      Phase 4.
- [x] Add/extend tests: frontmatter, seams, budget, the refusal wording; run

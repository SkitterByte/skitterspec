...
- [ ] **New Linear** — workspace, the team this repo files into, and where the
      team id comes from. State the one-team-per-repo limit (see
      `linear.config.md`) rather than leaving it to be discovered.
- [ ] **Install + MCP** — `npx @skitterbyte/skitterspec-linear init`, this **or**
...
- [ ] **Configure** — lead with `/spec-linear-setup`: what it asks (which team,
      ...
      never moves.
- [ ] **Configure by hand** — the same config as a clearly-secondary fallback for
      anyone not in Claude Code, flagged as skipping the state check.
- [ ] **The API key** — `skitterspec spec-sync credentials set`, run in your own
...
- [ ] **Check** — `spec-sync doctor` for the four offline layers, then
...
- [ ] **Fix** — `spec-sync retarget` for a renamed team (what breaks:
      `no Linear issue found for SKI-7`), and that `doctor` names the command for
      every other row.
- [ ] **Everyday sync** — `/spec-status`, `/spec-push`, and `/spec-sync` for the
      repo-wide operations.
- [ ] Verify every command shown is real: each `skitterspec spec-sync <verb>`
...
- [ ] Run the copy past `spec-sanitise` conventions — no emphasis or link

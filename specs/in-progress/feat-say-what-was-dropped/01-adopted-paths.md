...
- [ ] Add `manifestPresent(dir)` to `packages/common/src/init.js` — true only
      when the manifest file parsed and holds at least one entry. `readManifest`
      cannot answer this: it collapses missing and malformed into the same empty
      baseline.
- [ ] Extend `managedState` with `adopted`: the file exists, differs from
      `bundled`, has no manifest entry, **and** `manifestPresent(dir)`. Every
      other combination keeps its current answer — a cannot-tell stays
      `customized`, which keeps the file.
- [ ] Write the blind-spot comment beside it, naming both: a malformed manifest
...
- [ ] Add a `report.adopted` bucket carrying `{ relPath, added, removed, hunks }`
      — the same shape `report.customized` uses, so `--diff` needs no new branch.
      Reset it in `resetReport`.
- [ ] Handle `adopted` in `resyncManagedFile`: identical write behaviour to
      `customized` (keep unless `force`), recorded in the new bucket.
- [ ] Add the `printReport` line — `adopted upstream (kept — ours was never
      installed)` — and include the bucket in the `--diff` output and in the
      "re-run with --diff" hint.
- [ ] Add the `checkSync` row: `adopted upstream — yours kept, theirs not
      installed (--force takes theirs)`.
- [ ] Write the MIGRATION.md warning into the existing
...
- [ ] Add `packages/common/test/init-adopted-path.test.js`:
...
- [ ] Add the stays-silent tests to the same file (negative-checks rule 3):

...
- [ ] Register the skills in `src/init.js`: add `spec-env` and `spec-env-down` to
...
- [ ] Opt-in hooks (documented, gated — never forced): in `assets/skills/spec`
      ...
      as today.
- [ ] Cross-reference in `assets/rules/spec-planning.md` (or a short new rule):
      note the isolation skills and that they are opt-in and independent of the
      spec lifecycle status.
- [ ] Add a test asserting `SKILLS` includes both skills and that

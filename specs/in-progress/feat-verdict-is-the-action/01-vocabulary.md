...
- [ ] Rename `approve` → `commit` in `VERDICTS`, and add `commit-continue`.
      `DEFAULT_VERDICT` stays `discuss`: an absent verdict must keep meaning what
      it always meant (`feat-review-verdict` Decision 2).
- [ ] **Read a stored `approve` as `commit`** wherever a sidecar is loaded — the
...
- [ ] Block `commit-continue` on an open comment exactly as `commit` is blocked,
...
- [ ] Keep the refusal counting **comments only**. Unaccepted files still block
...
- [ ] **Stays silent:** a pass carrying no verdict behaves exactly as it does

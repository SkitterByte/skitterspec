...
- [x] Say on the continue button what it will not do: it builds the **next
      phase** and stops. A reader must not press it expecting the spec to be
      finished and landed.
- [x] Keep the bar **after the diff** (`feat-review-verdict`) and the send path
      unchanged (`feat-review-post-back`): a fourth button changes what is sent,
      not how.
- [x] Keep the layout honest on a phone — four controls wrap rather than
      shrinking to unreadable, and the two committing ones stay adjacent so the
      pair reads as a pair.
- [x] Tests, driving the real page as `assets-review.test.js` establishes: each
...
- [x] **Stays silent:** `file://` still copies, a served page still posts, and

...
- [ ] Add `TARBALL_INPUTS` — per package, the source paths that compose its
      ...
      added without one.
- [ ] Add a pure `lastTagFor(name, tags)` returning the highest existing tag for
      a package by semver, or null. Test it against an unsorted tag list and an
      empty one.
- [ ] Add an impure `changedInputs(root, name, sinceTag)` shelling out to
      `git diff --name-only <sinceTag> HEAD -- <inputs>`, filtering out the
      package.json version-only change (Decision 4).
- [ ] Add a pure `assertShippableChange(changed, { name, sinceTag, allowEmpty })`
      in the `assertCleanTree` idiom: throws naming the package and the tag it
      compared against, and points at `--allow-empty`.
- [ ] Wire `--allow-empty` through `parseArgs`; assert it defaults to false and
      that the existing flags still parse.
- [ ] Call the guard in `execute`, alongside `assertCleanTree` and
      `assertTagAvailable`, so it runs before anything is written.
- [ ] Regression-test the two real cases from the field report: the
...
- [ ] Document the refusal and `--allow-empty` in `RELEASING.md`.
- [ ] GREEN — full suite green. Commit with a `Release-Note:` — this changes

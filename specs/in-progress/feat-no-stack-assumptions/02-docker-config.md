...
- [x] Pass `-f <composeFile>` to the `docker compose … up -d` line in
      ...
      worktree.
- [x] Gate the no-header fallback in `provision.js` on the configured compose
      file existing, so `spec.stack` absent + `docker.enabled: true` +
      no compose file resolves to `worktree`.
- [x] Name the blind spot beside it: the check reads the compose file's path
...
- [x] Add tests: `-f` appears in both plans; a no-header spec with a compose

...
- [ ] Replace `detectPackageManager` with `detectRunner(dir)` in
...
- [ ] Filter `_npx` cache directories out of the PATH rung, and write the blind
      spot beside the check as a comment, per `negative-checks.md` rule 2.
- [ ] Return the rung alongside the prefix so the caller can warn without
      re-deriving it; keep `renderCommand(content, dir)` pure so
      `managedTargets` still compares against what `installCommands` writes.
- [ ] Handle the empty-runner case in the four command assets so rung 2 renders
      `Bash(skitterspec spec-env …:*)` rather than `Bash( skitterspec …:*)` —
      eight `{{exec}}` sites across `assets/commands/*.md`.
- [ ] Add a `report.warnings` entry on rung 3 naming the install command and
      that `update` re-renders once the CLI resolves.
- [ ] Extend `test/init-commands.test.js`: one test per rung; a stays-silent test

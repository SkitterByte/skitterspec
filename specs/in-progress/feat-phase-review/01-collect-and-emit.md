...
- [x] Add `packages/common/src/env/review.js`: resolve the spec (reuse
...
- [x] Collect with `git -C <worktreePath>` only. **Never** `cd`, never spawn
      inside the worktree: the whole feature exists because the caller's shell
      stays where it is.
- [x] Read `git status --porcelain` **without trimming the whole output** — the
...
- [x] Generate each patch with a large `-U` so the patch is the whole file, and
...
- [x] Include **untracked** files via `git diff --no-index -- /dev/null <path>`,

...
- [x] Add `packages/common/assets/skills/spec-diff/SKILL.md`. Resolution order:
...
- [x] **Gate it on nothing.** Not on tests passing, not on the phase being
...
- [x] Write the review as JSON for `spec-env review --review <file>`: a short
...
- [x] Say what it costs before spending it. The page itself is free; the written
      review is ~700 output tokens. On `--page-only`, render and stop.
- [x] Publish only when asked. On first publish, store the URL at
...
- [x] Degrade in one line when the harness cannot publish: report the local file
      path and carry on. The engine must stay ignorant of publishing entirely.
- [x] Mark the skill model-invocable so `/spec-next` can offer it, and say in the
      skill that publishing is never automatic — it leaves something behind that
      the tooling cannot remove.
- [x] Add `packages/common/test/assets-spec-diff.test.js`: the skill documents

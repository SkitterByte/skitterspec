...
- [x] Add an **intake** step to `assets/skills/spec-diff/SKILL.md`: on a pasted
      ...
      into auto-editing.
- [x] On the go-ahead: read **only** the commented files — never the accepted ones
...
- [x] State the intake's cost honestly alongside the existing two-cost section:
...
The intake went in as **§2**, before the render — a pasted blob *is* the
invocation, so discovering it after rendering a page nobody asked for would be
the wrong order. Sections below it renumbered; the guard test keys on prose, not
numbers.

The description gained one trigger — "pastes back the JSON the review page's Copy
button produced" — which is the phrasing that has to route, and it cost 6
characters of a 500-character budget.

The docs site needed a fourth pipeline stage (BUILD → LOOK → **HAND BACK** →
COMMIT), so its grid moved off a hardcoded three columns to four, with a
two-column step at 1100px.

...
- [ ] Add `spec-sync ref [--json]` to `packages/linear/src/cli-sync.js`: resolve
      the current git branch back to its spec and print that spec's
      `linear_identifier`.
- [ ] Resolve by **inverting `branch.pattern`**, not by guessing: for each spec
...
- [ ] Exit non-zero, with a reason, in each no-ref case: not on a spec branch
...
- [ ] Add `packages/linear/assets/rules/commit-trailers.md`: state the
...
- [ ] Verify the rule ships only in the Linear distribution: `pnpm build`, then
      assert it is present under `packages/skitterspec-linear/assets/rules/` and
      **absent** from `packages/skitterspec/assets/rules/`.
- [ ] Add `packages/linear/test/cli-ref.test.js`: a linked spec's branch resolves
...
- [ ] Extend `packages/linear/test/assets.test.js` to pin the rule's content —

...
- [ ] Update the two command files' frontmatter —
...
- [ ] Update `packages/common/assets/rules/spec-planning.md`: the live-overlay
...
- [ ] Update `docs/index.html`: the `spec-env connect` and `spec-env live` rows
      in the engine table, and the bare-argument note beneath it that currently
      calls out the two exceptions.
- [ ] Search the shipped assets for the old wording rather than trusting this
...
- [ ] Add a dated one-line entry to the **Changelog** of
...
- [ ] Run `node --test scripts/docs-claims.test.js` and the prose guards early

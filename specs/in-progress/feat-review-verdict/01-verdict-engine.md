...
- [x] Accept `verdict` in `validateNotesBlob`: `approve`, `changes` or
...
- [x] Treat an absent verdict as `discuss` at the point of *use*, not by
      rewriting the blob. The stored pass should record what was actually sent.
- [x] **Refuse `approve` while any comment is unresolved** — counting the notes
...
- [x] **Merge the comments anyway when an approval is refused.** The notes are
      good work regardless of the verdict being wrong, and losing them would
      punish the mistake twice.
- [x] Never block on **unaccepted files** — not here, not anywhere. Decision 3.
- [x] Add the outcome log: `decisions: [{ verdict, at, note }]` in the sidecar,
      appended when a verdict is acted on. History, not state — nothing reads it
      to decide anything (Decision 6).
- [x] Surface it in `--json`: the verdict as sent, whether it was honoured, and
...
- [x] Say the verdict in the human output, once, in the existing `notes:` block.
- [x] Tests: each verdict validates; an unknown one is refused; approve with an
...
- [x] Add the **stays-silent** case: a blob with no verdict produces byte-identical

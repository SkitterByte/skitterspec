...
- [x] Write the rule into `/spec-diff` §2, in the strongest terms the file uses:
      **never claim a pass the operator did not ask for.** Say why it is the only
      ...
      not prevent it.
- [x] Write the offer: name the **code**, the verdict and the age, and ask
      ...
      (Decision 4).
- [x] **Two or more waiting is a refusal to guess**, not a preference. Name them
      all and ask which; never take the newest, the oldest, or the only approve.
- [x] On a "no": leave it, and offer `--drop <code>`. A rogue pass that stays in
      the store is reported on every render until the operator stops reading the
      line.
- [x] Fix the page copy: it currently says `Sent · claim it with 386552`, which
...
- [x] Update `spec-planning.md`, the CLAUDE.md section and the docs site: the
...
- [x] Tests: the skill prose pins the never-unasked rule, the offer-by-code, and

...
- [x] Rewrite `every skill that changes spec state carries a tracker seam`
...
- [x] Populate all seven entries: `spec`, `spec-bug`, `spec-hotfix`, `spec-go`,
...
- [x] Assert every `lastWriteMarker` is actually **found** (`!== -1`) before
...
- [x] Add the **stays-silent test** required by
...
- [x] Keep `the skills that change no status carry no tracker seam` unchanged;
      `/spec-to-main` and `/spec-live` remain deliberately seam-free.
- [x] Name the blind spot beside the check (rule 2): this map is hand-maintained,

...
- [ ] Rewrite the block in `packages/common/assets/rules/spec-reports.md`: a
...
- [ ] **Rename `Diff` to `Review`** in the field vocabulary. One row carries the
      counts, the page link and the offer; there is no separate offer row, and
      it is present only when there is a rendered page.
- [ ] **Re-order the vocabulary** to decision 13: `Tracker` · `Why` · `Branch` ·
...
- [ ] Make `Tracker` the **first** row, carrying the id linked where the spec has
...
- [ ] Re-render all four worked examples, and the `❌` growth example, as tables.
- [ ] Update the 11 common skills' `**Fields:**` declarations: `Diff` becomes
      `Review`, moved last, on the four that render a page (`spec-next`,
      `spec-bug`, `spec-hotfix`, `spec-diff`).
- [ ] Rewrite `/spec-next` step 5, `/spec-bug` step 5b and `/spec-hotfix` step 6b:
...
- [ ] Rewrite `assets-offer-last.test.js` around the new anchor. It keeps its
...
- [ ] Teach `assets-report-contract.test.js` the new order, and that `Review`

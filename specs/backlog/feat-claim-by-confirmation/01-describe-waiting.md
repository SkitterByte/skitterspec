...
- [ ] Report each waiting pass on the render, not a bare count: its **code**, its
...
- [ ] Carry the same list in `--json` as `pending[]`, so a skill reads structured
      data rather than parsing the human line.
- [ ] Do **not** include the blob. The counts and the verdict are what a decision
      needs; the notes are what a claim is for, and reporting them here would put
      an unclaimed stranger's text into the context.
- [ ] Add `spec-env review <spec> --drop <code>` — the same match-by-code as
      `--claim`, with the same refusal and the same silence about what else is
      waiting. It removes and merges nothing.
- [ ] Keep the ordering stable and oldest-first, so two renders in a row name the
      passes in the same order and a reader can trust what they just read.
- [ ] Tests: the render names code, verdict and age; `--json` carries `pending[]`
...
- [ ] **Stays silent:** a spec with no holding area renders byte-identically to

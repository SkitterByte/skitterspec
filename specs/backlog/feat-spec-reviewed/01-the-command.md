...
- [ ] Write `packages/common/assets/skills/spec-reviewed/SKILL.md`. Skills install
...
- [ ] Mark it **`disable-model-invocation`**, and say in the body *why*: this is
...
- [ ] Resolve bare exactly as every other bare command does: the worktree you are
...
- [ ] On a no, leave it and offer `--drop <code>`.
- [ ] Nothing waiting is an ordinary answer, not a problem: say so, mention that a
      `file://` page copies to the clipboard instead, and stop.
- [ ] End with the report block (`.claude/rules/spec-reports.md`) and declare its
      fields, as every lifecycle skill does.
- [ ] Update `spec-init`'s list of the lifecycle skills it ships, the
      `spec-planning.md` skill table (marking it user-only beside `/spec-status`
      and the rest), the CLAUDE.md section, and the docs site.
- [ ] Keep the description inside the 500-char budget, with triggers that route
      the things an operator actually types — "I approved it", "I've reviewed it",
      "pick up my review".
- [ ] Tests: the existing per-skill guards cover it once it ships (description

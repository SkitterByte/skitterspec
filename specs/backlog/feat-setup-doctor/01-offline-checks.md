...
- [ ] Add `packages/linear/src/doctor.js` with `runChecks(state)` returning
      `{ ok, checks: [{ id, label, state, detail, fix }] }`, where `state` is
      `ok` · `missing` · `broken` · `skipped`.
- [ ] Distinguish **missing** (an opt-in not taken — fine) from **broken**
      (configured but wrong — not fine). `ok` is true only when nothing is
      `broken`, so declining an opt-in never reads as a failure.
- [ ] Cover the four layers: scaffold (`specs/` folders + skills present),
      isolation (`env.config.json` parses), tracker (`linear.config.json` parses
      and carries a `teamId`), key (present, with source).
- [ ] Take the project's state as an **argument**, gathered by the caller — keep
      this module free of `fs` so every branch is testable from a literal.
- [ ] Give each non-`ok` row a `fix` naming the exact command.
- [ ] Add `packages/linear/test/doctor.test.js` over literals: all-ok; each layer

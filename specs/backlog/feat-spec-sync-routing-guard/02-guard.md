...
- [ ] Add the guard to `scripts/docs-claims.test.js`, beside the existing
      both-ways engine checks. It already parses the docs pages **and** the
      shipped skills tree, so neither reader is new.
- [ ] Read each `spec-sync` row's **used-by cell** from `docs/linear.html` and
...
- [ ] Parse the routing table specifically — the `| ask | run |` rows — never the
...
- [ ] Name the blind spot in a comment beside the check: this is only as good as
...
- [ ] Write the failure message so it names the fix — route it, or correct the
      used-by cell — rather than only reporting the mismatch.
- [ ] Add the **stays-silent** tests (`.claude/rules/negative-checks.md` rule 3):
      ...
      nothing.
- [ ] Prove it fires: temporarily drop a routed row and confirm the suite goes

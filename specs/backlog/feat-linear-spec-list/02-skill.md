...
- [ ] Add `assets/skills/spec-list/SKILL.md` with frontmatter modelled on
...
- [ ] Document the opt-in gate — runs only when
      `specs/.core/linear.config.json` exists; otherwise say how to enable Linear
      sync and stop.
- [ ] Write the **API path**: run `spec-sync list` with the flags the user's
      question implies and relay its output.
- [ ] Write the **MCP path**: when the engine prints `transport = mcp`, discover
...
- [ ] Write the **degradation** step (decision 10): Linear unreachable or no

...
- [ ] For in-progress rows only, fetch the spec's phase sub-issues
...
- [ ] Render it as `<n>/<total> — <phase title>`, where `n` is the sub-issue's
      position in the phase order, so the row says both where the work is and how
      much is left.
- [ ] Handle the three real shapes without guessing: no sub-issue in progress
...
- [ ] Skip the lookup for every non-in-progress row, so a backlog-only listing
      makes exactly one Linear call (decision 8).
- [ ] Mirror the column in the `/spec-list` skill's MCP path and in `--json`.
- [ ] Extend `test/cli-list.test.js`: the phase column on an in-progress spec;

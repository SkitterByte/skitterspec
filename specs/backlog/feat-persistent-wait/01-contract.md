...
- [ ] Add the persistent-watch preference to the wait section: run the
...
- [ ] Re-word the re-arm paragraph: a wait that dies without a verdict
...
- [ ] Note why stdout is monitor-ready (heartbeat on stderr — SKS-358), so
      the next editor does not move it back.
- [ ] Update the asset-pinning tests that hold these sentences; add a

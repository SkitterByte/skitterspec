...
- [ ] In `planSpecCommit` (`packages/common/src/env/provision.js`), make the
...
- [ ] Surface them from `planUp` as `untouched` (or `foreign`) alongside
      `specCommit`, so the plan carries the fact rather than swallowing it.
- [ ] Print them in the `spec-env up` worktree renderer in
      `packages/common/src/cli.js`, next to the existing `specCommitLines`
      output, under a heading that says they are left alone.
- [ ] Leave `planCheckoutUp` untouched — it passes `carriesChanges: true` and
      keeps refusing.
- [ ] Extend the comment above `planSpecCommit` to name the new blind spot: this
...
- [ ] Add `Untouched` to the field vocabulary in
      ...
      one-line "Carries" entry.
- [ ] Declare `Untouched` in `/spec-start`'s `**Fields:**` line, in the rule's
      ...
      together).
- [ ] Rewrite `/spec-start`'s gate section
...
- [ ] Tests: worktree mode with foreign dirt provisions and reports it; checkout
...
- [ ] **Stays-silent test:** provisioning with a foreign path present exits zero

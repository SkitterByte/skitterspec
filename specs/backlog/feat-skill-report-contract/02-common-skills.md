...
- [ ] Rewrite `## Report` in each of the 11: `spec`, `spec-bug`, `spec-hotfix`,
...
- [ ] Keep the behaviour that currently sits inside those sections — *"Do not
...
- [ ] Give each skill its field list deliberately, not uniformly: `/spec-diff`
...
- [ ] Write `packages/common/test/assets-report-contract.test.js`: every skill
...
- [ ] Pair it with a stays-silent case (`.claude/rules/negative-checks.md`
...
- [ ] Re-check the skills that assert ordering — `assets-offer-last.test.js` and

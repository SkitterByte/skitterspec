...
- [ ] Add `PHASE_MAPPINGS = Object.freeze(['subissue', 'deferred'])` to
...
- [ ] Export `PHASE_MAPPINGS` alongside `TASK_MAPPINGS`.
- [ ] In `normalizeLocal` (`packages/sync-core/src/normalize.js`), compute
      `workflowState` **before** building `subIssues` — it is currently derived
      last, and the withhold rule needs it.
- [ ] Withhold when `mapping.phases === 'deferred'` and that status is `backlog`
      ...
      live sub-issue.
- [ ] Keep `## Phases` in the description when the withhold actually dropped a
      phase (Decision 5). Leave `phasesProjected` untouched in every other case
      so no existing spec's description hash moves.
- [ ] Tests in `packages/sync-core/test/` — cover: default `subissue` mode
...
- [ ] Test in `packages/linear/test/` that `mapping.phases: "nonsense"` throws
      and that `"deferred"` loads clean.
- [ ] Assert the transition end to end: a withheld backlog spec produces a plan

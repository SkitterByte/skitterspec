...
- [ ] Add `packages/linear/src/identity.js`: `resolveIdentity(config, env, deps)`
...
- [ ] Extend `credentials.js` with `userForTeam(store, teamId)` and
      ...
      `teams[<id>].key`.
- [ ] Add `viewer()` and `searchUsers(query, { limit, cursor })` to the API
...
- [ ] Declare the MCP counterparts in `mcp.js` so the skill path has named tools
      to reach for: `get_user { query: "me" }` and `list_users { query, limit }`.
- [ ] Add `spec-sync whoami [--json] [--set <id> --name <n>] [--unset]` to
...
- [ ] Add tests: derive-from-viewer; cached entry short-circuits the network;
      ...
      phase is done.
- [ ] **Stays-silent test** (`.claude/rules/negative-checks.md`): with no config,

...
- [ ] In §2.3 — *"on the go-ahead, work only the commented files"* — add the
      ...
      **name argument** is §1's first rule, so this is the ordinary case here,
      not an edge one.
- [ ] Record the baseline with `--record-primary` **before the first edit**, and
...
- [ ] Carry §4b's three verdicts across intact — `clean` carries on, `leaked`
      ...
      engine's output.
- [ ] **Do not touch §3, "Gate it on nothing".** State in the new text that this
...
- [ ] Leave the read-only paths untouched. Rendering, serving and `--notes`
      intake write nothing into a checkout, so none of them acquire a baseline,
      an assertion or a condition to evaluate.
- [ ] Add assertions to `packages/common/test/assets-spec-diff.test.js`: the
...
- [ ] Add the **stays-silent** cases: standing in the spec's own worktree leaves
...
- [ ] Run `pnpm test` at the repo root — green before the phase is done, then
      `pnpm build` to refresh the composed copy.

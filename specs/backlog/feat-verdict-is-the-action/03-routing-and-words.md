...
- [ ] Keep `changes` exactly as it is: it is the go-ahead, and this spec does not
      touch it beyond the vocabulary around it.
- [ ] Carry the routing into **`/spec-reviewed`** if that spec has landed; if it
...
- [ ] Update `spec-planning.md`, the CLAUDE.md section, `env.config.md` and the
...
- [ ] Record the overturned Non-goal where a reader will meet it: the chaining
      ban was right about *automatic* chaining and wrong to catch a chosen one.
      Cite it rather than quietly contradicting it.
- [ ] Keep the `/spec-diff` description inside the 500-char budget.
- [ ] Tests: the skill prose pins the commit-then-next routing, the stop at the

...
- [ ] Add `review.commitWith` to `env.config.json` (default `"/commit"`), through
...
- [ ] Decide availability from **the skill list already in context**, never by
...
- [ ] Let the commit's own failure be the answer: if typecheck or tests fail, the
...
- [ ] Append the outcome (`approve`, the commit sha, which path) to the decisions
      log, then re-render so the page shows it.
- [ ] Tests: the configured skill is named in the hand-off; `"none"` records and

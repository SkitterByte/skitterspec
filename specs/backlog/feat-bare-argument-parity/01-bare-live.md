...
- [ ] Give `liveGrammar` a no-argument arm in `packages/common/src/cli.js`. It
...
- [ ] **Decide from two positive signals, never from an absence.** Take only when
...
- [ ] Keep the fallback **silent about why** only where the report already says
...
- [ ] Do not duplicate the guards. `specEnvLiveTake` already refuses a dirty
...
- [ ] Leave `live status` untouched, including its own zero-arg meaning (the
      repo-wide report). A missing spec after an explicit verb is a different
      question from a missing verb.
- [ ] Rewrite the stays-silent guard in
...
- [ ] Add tests in `packages/common/test/cli-spec-env-live.test.js`: bare takes

...
- [ ] Render it above `#files` in `page.html`: the problem, the Impact table,
...
- [ ] **A spec that cannot be parsed yields no header, not an empty one** — a
...
- [ ] Tests, driving the real page as `assets-review.test.js` establishes: the
      ...
      and reachable.
- [ ] **Stays silent** (`.claude/rules/negative-checks.md` rule 3): a spec with

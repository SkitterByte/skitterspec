...
- [x] Add `fix` to `BUTTON_SETS` in `packages/common/src/env/review.js`.
- [x] Add `fix: ['commit', 'changes', 'discuss']` to the `OFFERS` table in
...
- [x] Add `fix` to `NARROWING_SETS` in `packages/common/src/env/serve.js`, and
...
- [x] Test: a `--buttons fix` render offers exactly those three, and hides
...
- [x] Test: the served page keeps `fix`. Extend
...
- [x] Test (stays silent): a record saying `fix` for a spec whose tree is

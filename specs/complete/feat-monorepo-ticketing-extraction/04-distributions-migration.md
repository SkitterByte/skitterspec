...
> Spec: [00-overview.md](00-overview.md) · **Status:** Done (build complete; the
> port-branch deletion is the one post-landing task, done at `/spec-complete`)
...
- [x] Make `packages/common/src/init.js` **asset-driven**: discover skills / rules /
      ...
      wiring untouched. Update `init.test.js` accordingly. *(No test change needed —
      `init.test.js` already asserts via the `SKILLS`/`RULES` exports and the
      discovery yields exactly the same base set.)*
- [x] Port `scripts/build-dist.js` (+ `scripts/build-dist.test.js`): compose
...
- [x] Create `packages/skitterspec` (published, **2.0.0**) — `files: [bin, src,
...
- [x] Create `packages/skitterspec-linear` (published, **1.0.0**) — superset bin +
...
- [x] Write `MIGRATION.md` — v1→v2 split: no-op if you didn't use Linear; one
...
- [x] Root `npm run build` builds both; verify `npm pack --dry-run` for each ships
      the composed tree with no private-package dep. *(base → dep `prompts` only,
      29 files; superset → dep `prompts` only, 45 files with engine/adapter vendored
      under `src/vendor/`.)*
- [x] Add the hermetic smoke test: build each dist, copy it OUTSIDE the workspace

...
- [x] Rewrite the hero: headline sells spec-driven dev + the review loop
      (direction: "The spec is the plan. The page is the review. Git is the
      board."); sub-copy keeps the grilled-spec → landed-code arc.
- [x] Replace the hero's Linear card with a compact review-page mockup
      (reuse phase 2's classes): terminal mockup → review page with verdict
      buttons.
- [x] Demote Linear in the hero to one line + link ("tracker-optional — add
      Linear →"), keep the #linear section below as the superset's on-page
      summary.
- [x] Drop `+ linear` from the brand lockup.
- [x] Build the review teaser section replacing the old #review wall: one
      ...
      review.html.
- [x] Re-order the page: hero → loop → walkthrough → report → review teaser
      → Linear summary → quickstart; update the nav anchors to match.
- [x] Sweep the walkthrough and remaining copy for `spec-env` mentions —

...
- [x] Remove the **`open.command`** config key entirely: the default in
...
- [x] Drop the `open` block from this repo's own
...
- [x] Collapse `/spec-start` step 3 in
...
- [x] Keep the worktree **trust** step. It is not tab machinery: worktrees live
...
- [x] Rewrite `packages/common/test/assets-spec-start-enter.test.js` to guard the

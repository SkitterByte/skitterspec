...
- [x] `/spec-next` §5 step 2: "run the engine's wait in the background" →
...
- [x] `/spec` Phase C2 "Then wait for it": same sentence, same deference.
- [x] `/spec-bug` 5b and `/no-spec`: confirm they delegate to `/spec-next`
      §5 / the rule; update any restated sentence rather than adding a copy.
- [x] Update the asset-pinning tests that hold the skills' wait sentences.
- [x] Run `skitterspec update` in this repo (accept managed changes) so the

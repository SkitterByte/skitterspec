...
- [x] RED — extend `sync-task-checklist.test.js`: a phase file with a
      ...
      `[ ]` invented.
- [x] Branch on `b.checkbox` in `normalize.js:350` — checkbox blocks keep
      `- [x]`/`- [ ]`, non-checkbox blocks render `- ` only. Keep the existing
      inline `(KEY-123)` stripping for both.
- [x] Skip non-checkbox blocks in `write.js` `stampIssueId` — it is the legacy
      per-task stamp and must never append an id to a plain bullet.
- [x] Audit every other `findTaskBlocks` caller for the same assumption
      (`write.js:130`, `normalize.js:350`, the test helpers) and fix or assert
      each.
- [x] Confirm the churn is `update`-only, never `create`: run
...
- [x] GREEN — full suite green; add a `Release-Note:` on the commit, since the

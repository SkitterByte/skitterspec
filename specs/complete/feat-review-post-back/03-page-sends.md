...
- [x] Send on verdict: POST the blob to `location.pathname` — the page's own URL,
      so there is no second address to configure (Decision 8).
- [x] Decide the path from what the page **is**, not from what it can guess:
...
- [x] Show the code where the verdict was pressed: `sent · claim it with 418207`,
      in the row the buttons are in, with the code selectable for anyone who
      would rather copy it than read it out.
- [x] Keep the clipboard fallback intact for `file://` — same textarea, same
      hint, no deprecation notice (Decision 7).
- [x] Report a refused POST on the page, with the engine's message. A pass the
      server rejected must not read as sent.
- [x] Leave the marks in place after sending. A pass is not spent until it is
      claimed, so a page that cleared itself would be lying about state it does
      not own.
- [x] Tests: driving the real page as `assets-review.test.js` establishes — a

...
- [x] Teach `routeFor` the method. It currently ignores `req.method` entirely, so
      a POST renders HTML — that absence is why the paste was the only way in.
- [x] Handle `POST /<token>/<spec>`: read the body with a **size cap**, parse it,
...
- [x] Write it to the pending store from phase 1 and answer `200` with
      `{ code }`. Nothing else is written, and the notes sidecar is untouched.
- [x] Refuse a POST to the index route and to an unknown spec, with the same 404
      the GET gives — a write path must not become a way to enumerate what exists.
- [x] Cap and reject: a body over the cap, a body that is not JSON, and a blob
      naming a different spec than the URL, each with a distinct message.
- [x] Tests: a POST stores exactly one pending pass and returns its code; a bad
...
- [x] **Stays silent:** the whole existing GET suite passes untouched. A server

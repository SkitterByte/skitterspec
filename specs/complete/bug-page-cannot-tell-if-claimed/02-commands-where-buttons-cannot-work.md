...
- [x] `/spec-reviewed <verdict>` — the skill accepts a verdict word alongside
...
- [x] `spec-env review <spec> --verdict <word>` underneath it, joining the same
      merge a claim goes through so nothing downstream can tell them apart.
- [x] On a `file://` page, replace the verdict bar with a stacked list — one row
...
- [x] Copy puts the command on the clipboard and then shows the same You chose
...
- [x] No clipboard (the ordinary `file://` case) → the row selects its command
      and says so, exactly as the served path already does.
- [x] **Marks do not fit in a command line.** The moment the pass carries an
...
- [x] Tests green: the `file://` half of `assets-review.test.js`, the skill

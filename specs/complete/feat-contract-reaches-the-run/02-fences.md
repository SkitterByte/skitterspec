...
- [x] Rewrite `/spec-start` step 6's *"worktree ready … build phase 1 now?"*
...
- [x] Rewrite `/spec-list` step 5's *"start one with: /spec-start <name>"*
      hand-off as a sentence.
- [x] Add the rule to `packages/common/assets/rules/spec-reports.md`: a fenced
...
- [x] Write `packages/common/test/assets-fences.test.js`. For every shipped
...
- [x] Build the allowlist from what actually ships today, one entry per block
...
- [x] Pair it with a stays-silent case: a command block, a JSON block and an
...
- [x] Assert the allowlist is **not vacuous and not stale** — every entry names a

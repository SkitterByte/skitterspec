...
- [x] Add a function that compares the block between `<!-- skitterspec:start -->`
...
- [x] Add `skitterspec update --check`: report per managed area plus the
      `CLAUDE.md` section, **write nothing, exit 0**. It reports; `update`
      without the flag is still the only thing that changes a file.
- [x] Word the `differs` case as a difference, never as staleness — the user's
...
- [x] Name the blind spot in a comment beside the check: markers absent means the
      section was never installed **or** was deliberately stripped
      (`stripClaudeMdSection` exists), and neither is a fault.
- [x] Test all three states against scratch projects: a freshly installed project
      ...
      exit 0.
- [x] Pair it with the stays-silent case that matters most: a project whose

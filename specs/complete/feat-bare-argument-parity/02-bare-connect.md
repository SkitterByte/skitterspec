...
- [x] Remove `const target = specArg || 'main'` from `specEnvConnect` in
...
- [x] Keep `main` and the configured base branch as the disconnect form,
      including the literal `main` in a repo whose base is named something else
      — the same courtesy `liveGrammar` already extends.
- [x] Let ambiguity **refuse**, not fall back. Unlike `live`, `connect` has no
...
- [x] Invert the stays-silent guard in
      ...
      wrong.
- [x] Add tests: bare connects the sole provisioned spec; bare connects the spec

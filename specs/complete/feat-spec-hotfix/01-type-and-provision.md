...
- [x] `resolve.js` `splitPrefix`: accept `hotfix` — regex `^(feat|bug|hotfix)-(.+)$`
      so a `hotfix-<slug>` folder resolves to `{ type: 'hotfix', slug }` and the
      branch pattern yields `hotfix/<slug>`.
- [x] `resolve.js` `resolveSpec`: read the `> **Base version:** <tag>` blockquote
...
- [x] `provision.js` `planUp`: when `spec.baseRef` is set, emit
...
- [x] `live.js` `take` planner: refuse when `spec.type === 'hotfix'` with a clear
...
- [x] Add/extend tests (`*.test.js` beside each module, `node --test`): prefix
...
- [x] Run `pnpm test` — green before the phase is done. **242 pass, 0 fail.**

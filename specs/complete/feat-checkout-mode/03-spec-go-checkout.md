...
- [x] Teach `spec-env up` a checkout-mode plan: create the spec's branch in the
...
- [x] Require a clean tree before switching, and refuse with the reason when it
      is dirty — `git switch -c` would otherwise carry uncommitted work onto the
      new branch.
- [x] Refuse when the checkout is already on **another spec's** branch, naming
...
- [x] Amend `packages/common/assets/skills/spec-go/SKILL.md` so the provisioning
...
- [x] Keep the live check ahead of both paths — in checkout mode `live: yes` is
      impossible by construction, so state that rather than leaving a check that
      can never fire.
- [x] Surface the resolved `mode` in `spec-env up`'s plan output, beside
...
- [x] Add/extend tests covering this phase: the checkout-mode plan emits a branch

...
- [x] Amend `packages/common/assets/skills/spec-go/SKILL.md`: at the hand-off,
...
- [x] State the empty case in the skill: with `open.command` unset there is
      nothing to run, so print the path and ask the operator to open a session
      themselves — today's wording, unchanged.
- [x] Do not run an opener in a non-interactive context (no TTY / a scripted
      run) — opening a window nobody is sitting at is noise. Say so in the skill.
- [x] Note in `packages/common/assets/core/env.config.md` that a non-empty
...
- [x] Add/extend tests covering this phase: assert `spec-go`'s asset tells the

...
- [x] `/spec-next` §5 ("Then wait for the verdict, where the harness can"):
...
- [x] `/spec-bug` §5b and `/spec-diff` §4b: point at `/spec-next` §5 rather than
      restating it. Two copies of a waiting rule is how the two come to disagree.
- [x] Keep *"where the harness cannot watch, the turn ending is the wait"*
...
- [x] `.claude/rules/spec-reports.md`, where the banner's *"I'm holding here
...
- [x] **Say what a hand-rolled watcher costs**, once, where the rule is stated:
...
- [x] Guard test (`packages/common/test/assets-verdict-wait.test.js`): assert
...
**The guard had to be proved it could fire.** It passed on its first run,
because the documents were edited before it was written — and a guard that has
never been red proves nothing about whether it can be. Each assertion was run
against the mutation it exists to catch: the command removed, a `--timeout`
added, the reasoning stripped. All three went red and all three went quiet on
restore.

**Two of the edits tripped existing guards**, which is the repo's own rules
working. A `**bold**` span crossing a hard line break, twice — the wrap rule in
`.claude/rules/spec-planning.md`. And `assets-offer-last` asserts `/spec-next`
says **end your turn**; rewording it to "End your turn once it is running" broke
a property test about chained runs, so the phrase was restored rather than the
test loosened.

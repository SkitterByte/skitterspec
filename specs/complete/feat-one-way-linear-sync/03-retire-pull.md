...
- [x] Delete the content-pull path: `pull.js`, and the `write.js` pull-writeback
...
- [x] Repurpose `/spec-status` (skill + `spec-sync status`) to a **read-only
...
- [x] Remove `/spec-pull` (skill dir + `spec-sync pull` subcommand + docs/refs).
...
- [x] Demote `spec-sanitise`: keep the command as an **optional cosmetic** tidy
...
- [x] Docs + release: update `linear.config.json.example` (states + drop pull
...
- [x] Cancel/fold the pending `hotfix-sanitise-hyphen-join` spec (its fix is
      absorbed here) and tear down its worktree.
- [x] Full suite green (`node --test`) with the dead code removed; net line count

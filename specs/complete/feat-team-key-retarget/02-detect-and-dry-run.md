...
- [x] Add the `retarget` case to `cli-sync.js`: resolve the recorded key via
...
- [x] Spot-check before reporting: resolve the first remapped identifier and
      compare its title against that spec's — report `resolves, title matches`,
      or refuse with what was found instead.
- [x] Print the plan in the shape agreed in the overview: recorded vs live key,
      counts per category, the spot-check line, and the `--yes` hint.
- [x] Handle the MCP path, where `get_team` does not return the key: say so and
...
- [x] Refuse with a clear message when `teamId` is unset, or when
      `deriveRecordedKey` reports ambiguous stamps.
- [x] Add `packages/linear/test/cli-retarget.test.js` with a stubbed adapter:
...
- [x] Repoint the `/spec-sync` skill's section 7 and routing row at `retarget` —
      it documented `doctor --write`, so shipping this phase alone would have
      left the skill describing a command that no longer exists.
- [x] Delete `packages/linear/src/doctor.js` and `cli-doctor.test.js` now that
      `cli-sync.js` imports the planner from `sync-core` (deferred from phase 1).

...
- [x] **config.js** — add `setup: []` to `DEFAULT_CONFIG` and to `defaults()`.
...
- [x] **provision.js** — in `planUp`, expand each `config.setup` command via
...
- [x] **cli.js** — in `specEnvUp`, after the `run these:` block and before the
      `.env` block, print a `in the worktree, run:` heading listing
      `plan.setupCommands` (skip the heading entirely when the list is empty).
- [x] **Tests** — extend `test/env-config.test.js` (setup normalisation:
      ...
      green before the phase is done. **248/248 pass.**

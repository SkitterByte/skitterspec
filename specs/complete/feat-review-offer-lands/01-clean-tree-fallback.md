...
- [x] Append the reason to the page subtitle in
      ...
      not a new field.
- [x] Update `/spec-diff`'s SKILL.md where it states the working-tree default
      (`assets/skills/spec-diff/SKILL.md`, the `--branch` block) so the skill
      describes the fallback rather than contradicting it.
- [x] Tests in `packages/common/test/env-review-fallback.test.js` (**not**
      `env-review.test.js` — see Notes): the fallback fires on a committed
      branch with a clean tree and reports `branch` mode; the header line names
      the swap; the page subtitle carries the reason.
- [x] **Stays-silent tests** (`.claude/rules/negative-checks.md` rule 3): a spec
...
Seven tests, all green (`packages/common/test/env-review-fallback.test.js`);
full suite 1865 pass, 0 fail. Three of the seven are the fires-correctly half and
four are stays-silent.

**Deviation — where the tests live.** The phase named
`packages/common/test/env-review.test.js`. That file unit-tests `collectReview`
against a git fixture, and the fallback is not in the collector: it is the
decision in `specEnvReview` about *which mode to collect in*. Testing it there
would have tested the thing the decision calls. The CLI-level harness
(`run(['spec-env', 'review', …, '--dir', dir])`) already existed in
`env-review-notes.test.js`, so the new file reuses that shape and keeps one
concern per file, as the rest of `packages/common/test/` does.
